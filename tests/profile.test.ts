...
} from "../url-to-markdown/scripts/lib/browser/profile";

const originalProfile = process.env.URL_TO_MARKDOWN_CHROME_PROFILE_DIR;

afterEach(() => {
  if (originalProfile === undefined) {
    delete process.env.URL_TO_MARKDOWN_CHROME_PROFILE_DIR;
  } else {
    process.env.URL_TO_MARKDOWN_CHROME_PROFILE_DIR = originalProfile;
  }
});

describe("resolveChromeProfileDir", () => {
  test("uses URL_TO_MARKDOWN_CHROME_PROFILE_DIR when set", () => {
    process.env.URL_TO_MARKDOWN_CHROME_PROFILE_DIR = "/tmp/url-to-markdown-profile";
    expect(resolveChromeProfileDir()).toBe("/tmp/url-to-markdown-profile");
  });

  test("falls back to shared url-to-markdown profile path", () => {
    delete process.env.URL_TO_MARKDOWN_CHROME_PROFILE_DIR;
    const resolved = resolveChromeProfileDir();
    if (process.platform === "darwin") {
      expect(resolved).toBe(path.join(os.homedir(), "Library", "Application Support", "url-to-markdown", "chrome-profile"));
    } else if (process.platform === "win32") {
      expect(resolved.endsWith(path.join("url-to-markdown", "chrome-profile"))).toBe(true);
    } else {
      expect(resolved.endsWith(path.join("url-to-markdown", "chrome-profile"))).toBe(true);
    }
  });
...
describe("ensureChromeProfileDir", () => {
  test("creates the profile directory when missing", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "url-to-markdown-profile-"));
