...
Headless Chrome can silently return low-quality content — layout shells, login walls, or framework payloads — without the CLI returning a non-zero exit code. Read this after every headless run and inspect either the Markdown output or the JSON `markdown` field so you can catch and recover from those cases.
...
| `--headless` | Explicit headless (same as default) | The user explicitly requests headless capture |
